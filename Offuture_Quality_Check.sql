...
-- ORDER_id, customer_id/ customer_name AND product_id will make a unique row or 
-- order id and product_id will give a unique row
-- individual row is a transactions (row_id) orders is distinct(order_id)
-- Customer ID per customer name as it is not unique, group by customer name as name is unique
-- Africa, EMEA and APAC seperate id| LATAM, US and EU seperate id difference of 10000
SELECT 
	"Customer Name" ,
	count(DISTINCT("Customer ID")), 
	count("Order ID")
FROM team_having_offuture tho 
